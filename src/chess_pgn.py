...
class ChessPGNApp:
    ...
    def print_welcome(self):
        ...
        print("Type 'help' anytime for guidance | Type 'quit' to exit\n")
    ...
    def print_metadata_help(self):
        """Print help message for metadata entry."""
        print("\n" + "-" * 60)
        print("GAME INFORMATION FIELDS:")
        print("-" * 60)
        print("  Event  - Name of tournament or match (e.g., 'Club Championship')")
        print("  Site   - Location (e.g., 'New York' or 'Chess.com')")
        print("  Date   - Game date in YYYY.MM.DD format")
        print("  Round  - Round number (use 1 for casual games)")
        print("  White  - White player's name")
        print("  Black  - Black player's name")
        print("\nTip: Press Enter to use default values shown in [brackets]")
        print("Type 'quit' to exit without starting a game")
        print("-" * 60 + "\n")

    def _get_metadata_field(self, prompt: str, default: str = "") -> str:
        """
        Get a metadata field from user with help support.
        ...
        while True:
            ...
            if value.lower() == "help":
                self.print_metadata_help()
                continue
            if value.lower() in ["quit", "exit"]:
                print("\nExiting without starting a game.")
    ...
    def collect_metadata(self) -> dict:
        ...
        """
        print("Enter game information (type 'help' for field descriptions):\n")

        event = self._get_metadata_field("Event [Casual Game]: ", "Casual Game")
        site = self._get_metadata_field("Site [Local]: ", "Local")

        date_str = self._get_metadata_field("Date (YYYY.MM.DD) [press Enter for today]: ")
        if not date_str:
            today = date.today()
            date_str = f"{today.year}.{today.month:02d}.{today.day:02d}"

        round_num = self._get_metadata_field("Round [1]: ", "1")
        white = self._get_metadata_field("White player: ", "Player 1")
        black = self._get_metadata_field("Black player: ", "Player 2")
    ...
    def enter_moves(self):
        ...
        while True:
            if not self._enter_move_pair(move_number):
                break
            move_number += 1

        self.game.finalize_pending_move()

    def _get_validated_move(self, color: str, move_number: int) -> str:
        """
        Get and validate a move from user, handling commands and retries.
        ...
        Returns:
            Valid move string, or empty string if user quit
        """
        while True:
            move_input = self.get_move_input(color, move_number)

            if move_input.lower() in ["quit", "exit", "done"]:
                return ""

            if move_input.lower() in ["undo", "show", "preview", "legal", "help"]:
                result = self.handle_command(move_input)
                if result == "quit":
                    return ""
                continue

            success, message = self.game.validate_and_add_move(move_input, color)
            if success:
                return move_input
            else:
    ...
    def _display_move_confirmation(self, color: str, move: str):
        """Display move confirmation with board state."""
        print(f"\033[A\033[K{color.capitalize()}'s move: {move} ✓")

        board_state = self.game.get_board_state()
        if board_state:
            print(f"  {board_state}")

    def _complete_pending_black_move(self, move_number: int) -> bool:
        """
        Complete a pending white move by getting black's move.

        Args:
            move_number: Current move number

        Returns:
            True if move was completed, False if user quit
        """
        black_move = self._get_validated_move("black", move_number)
        if not black_move:
            return False

        self._display_move_confirmation("black", black_move)
        return not self.game.is_game_over()

    def _enter_move_pair(self, move_number: int) -> bool:
        """
        Enter a complete move pair (white and black).

        Args:
            move_number: Current move number

        Returns:
            True to continue, False if user quit or game over
        """
        white_move = self._get_validated_move("white", move_number)
        if not white_move:
            self.game.finalize_pending_move()
            return False

        self._display_move_confirmation("white", white_move)
    ...
    def add_more_moves(self):
        ...
        if has_pending:
            ...
            if not self._complete_pending_black_move(move_number):
                return
            move_number += 1
        else:
            move_number = move_count + 1
    ...
    def _get_edit_choice(self, move_count: int) -> str:
        """
        Get user's choice for editing or adding moves.

        Args:
            move_count: Number of existing moves

        Returns:
            User's choice ('done', 'add', or move number as string)
        """
        if move_count > 0:
            prompt = f"Enter move number to edit (1-{move_count}), 'add' to add more moves, or 'done' to finish: "
        else:
            prompt = "Enter 'add' to add moves, or 'done' to finish: "

        return input(prompt).strip()

    def _edit_single_move(self, move_num: int, move_count: int) -> bool:
        """
        Edit a single move at specified position.

        Args:
            move_num: Move number to edit
            move_count: Total number of moves

        Returns:
            True if edit was successful or cancelled, False to retry
        """
    ...
    def preview_and_edit(self) -> bool:
        ...
        while True:
            ...
            move_count = self.game.get_move_count()
            edit_choice = self._get_edit_choice(move_count)
            ...
            try:
                ...
                self._edit_single_move(move_num, move_count)
            except ValueError:
    ...
    def run(self):
        ...
        try:
            ...
            if self.game and (self.game.get_move_count() > 0 or self.game.has_pending_white_move()):
                ...
                try:
                    save_choice = (
                        input("Would you like to save before exiting? (y/n): ").strip().lower()
                    )
