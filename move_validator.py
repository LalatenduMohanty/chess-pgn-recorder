    ...
    def validate_move(self, move: str) -> Tuple[bool, str]:
        ...
        if len(move) > 0 and move[0] in 'kqrbn':
            if not (len(move) == 2 and move[1] in '12345678'):
                ...
                return False, f"Piece notation must be uppercase (use '{piece_upper}{move[1:]}' instead of '{move}')"
        ...
        if '=' in move:
            ...
            if promo_match:
                ...
                if promo_piece not in 'QRBN':
                    return False, f"Invalid promotion piece '{promo_piece}': can only promote to Q, R, B, or N"
            else:
